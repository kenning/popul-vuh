...
public class GridUnit : MonoBehaviour {
  ...
  private float pokeTime;
	...
	public void GridMove(string direction) {
		...
		if(!isMoving){
			switch(direction) {
			case "left":
				endPosition = new Vector3(gameObject.transform.position.x - 1, 
  ...
  public void PokeTowards(string direction)
  {
    if(!isPoking){
			switch(direction) {
			case "left":
				endPokePosition = new Vector3(gameObject.transform.position.x - 1, 
                                      gameObject.transform.position.y, 0);
				break;
			case "right":
        endPokePosition = new Vector3(gameObject.transform.position.x + 1, 
				...
				break;
			case "up":
        endPokePosition = new Vector3(gameObject.transform.position.x, 
                                      gameObject.transform.position.y + 1, 0);
				break;
			case "down":
        endPokePosition = new Vector3(gameObject.transform.position.x, 
  ...
  {
      if((Mathf.Abs(RelativeUnitCoords[0] - BaseCoords[0]) + 
          Mathf.Abs(RelativeUnitCoords[1] - BaseCoords[1])) != 1) {
