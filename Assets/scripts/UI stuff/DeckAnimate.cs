	...
	public void Update() {
		...
		if(ShuffleAnimating) {
			if(Time.time > AnimateStartTime + 1f) {
                ...
                battleBoss.gameObject.GetComponent<ShopControl>().shufflin = false;
                battleBoss.Tooltip = "";

				//VVV actually really important part. this is where the new level starts!
				if(SaveData.UnlockedGods.Count == 7) battleBoss.ReturnToGodChoiceMenu ();
				else battleBoss.gameObject.GetComponent<MainMenu>().BeginButton();
				return;
			}
			else {
...
//				cardUnderDeck.transform.localPosition = new Vector3(Mathf.Abs(Mathf.Cos(Time.time*2)), 2,0);
	...
	void Shuffle () {
		battleBoss.ShuffleInHandAndDiscard();
    ...
    bool hungerCheck()
    {
        return (battleBoss.HungerTurns > 0);
    }
}
